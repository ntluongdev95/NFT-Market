import React, { useEffect, useState } from 'react'
import '../styles/explore.scss'
import DoneIcon from '@mui/icons-material/Done';
import SearchIcon from '@mui/icons-material/Search';
import KeyboardArrowDownIcon from '@mui/icons-material/KeyboardArrowDown';
import FavoriteIcon from '@mui/icons-material/Favorite';
import { useDispatch, useSelector } from 'react-redux';
import {  toast } from 'react-toastify';
import axios from 'axios'
function Explore() {
    const[NFTS,setNFTS] = useState([])
    const [loadNFT,setLoadNFT]= useState(false)
    const[canceled,setCanceled] = useState(false)
    const{contract_MM_Market,contract_MM_Token,web3,account} = useSelector(state=>state.blockchain)
    const dispatch = useDispatch()
    useEffect(async()=>{
        if(account ){
      const data =  await contract_MM_Market?.methods.getMarketItems().call()
      const items = await Promise.all(data.map(async i =>{
          const tokenUri = await contract_MM_Token?.methods.tokenURI(i.tokenId).call()
          const meta = await axios.get(tokenUri)
          const price = await web3?.utils.fromWei(i.price.toString(),'ether')
          const item={
              price,
              name:meta.data.name,
              tokenId:i.tokenId,
              seller:i.seller,
              owner :i.owner,
              image:meta.data.image
          }
          return item
      }))
      toast.success('You just create your own NFT')
      setNFTS(items)
    }
    },[dispatch,account,loadNFT,canceled])
    const handleBuyNFT =async(nft)=>{
      const priceToW = await web3.utils.toWei(nft.price.toString(),'ether')
      await contract_MM_Market.methods.Sale((nft.tokenId)).send({from:account,
        value:priceToW
     })
       setLoadNFT(true)
       toast.success('You just bought successfully')
    }
    const handleCancelNFT = async(nft)=>{
        await contract_MM_Market.methods.Cancell((nft.tokenId)).send({from:account
         })

         setCanceled(true)
    }
    return (
        <div className='explore__wrapper'>
            <div className='explore__header'>
                <div className='header__banner'></div>
                <div className='header__info'>
                    <div className='image__logo'>
                        <img src='https://lh3.googleusercontent.com/1LvJD420yvelZntqhS0Bp9hc3Px25wKFKZ1kD5hq5jB5Dvl4Q7OnLWEzytzoCg6zUhkl63ev3WNr4BsDPWO6yRL8h4Ce4nRFN9kI=s0' alt='' />
                    </div>
                    <div className='header__account'>
                        <div className='account__name'>VN -NFTS MARKET</div>
                        <div className='tick__icon'><DoneIcon style={{color:'white'}}/></div>
                    </div>
                    <div className='account__btn'>
                        <img className='eth__logo' src='data:image/png;base64,' alt='' />
                        <div className='account__number'>0x2248...98D</div>
                    </div>
                </div>
            </div>
            <div className='nft__list'>
                <div className='nft__left'></div>
                <div className='nft__right'>
                    <div className='nft__right-top'>
                        <div className='nft__search'>
                            <SearchIcon />
                            <input className='nft__input' placeholder='Search' />
                        </div>
                        <div className='nft__filter'>
                            <span className='title'>All items</span>
                            <KeyboardArrowDownIcon />
                        </div>
                        <div className='nft__filter'>
                            <span className='title'>Filter</span>
                            <KeyboardArrowDownIcon />
                        </div>
                    </div>
                    <div className='nft__grid'>
                        {NFTS?.map(nft=>(
                    <div  key={nft.tokenId}  className='nft__card'>
                        <div className='image__container'>
                            <img className='nft__image' alt='' src={nft.image} />
                        </div>
                        <div className='info__container'>
                            <div className='nft__info'>
                               <div className='nft__id'>#{nft.tokenId}</div>
                               <div className='nft__price'>
                                    <small style={{color:'grey'}}> Last</small> <img src='data:image/png;base64,' />
                                    <span> {nft.price}</span>
                               </div>
                            </div>
                            <div className='nft__name'>{nft.name} </div>
                        </div>
                        <div className='nft__buy'>
                            {account.toLowerCase() ===nft.seller.toLowerCase() ? (
                                 <div onClick={()=>handleCancelNFT(nft)}  className='buy__btn'>Cancel</div>
                            ):(
                                <div  onClick={()=>handleBuyNFT(nft)} className='buy__btn'>Buy now</div>
                            )}
                            {account.toLowerCase() ===nft.seller.toLowerCase() ? (
                                <div className='nft__fav'> Update</div>
                            ):(
                                 <div  className='nft__fav'> <FavoriteIcon  style={{color:'grey'}}/> 6</div>
                            )}
                           
                        </div>
                    </div>
                    ))}
                </div>
                </div>  
            </div>
        </div>
    )
}

export default Explore
